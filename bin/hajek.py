...
def get_data(filename):
    ...
    return [[float(i[0])*10, float(i[1])*g] for i in data]
...
if __name__ == "__main__":
    ...
    w = wsum/len(results)
    print "----------------"
    print "Mean waist results: %s" % filename
    print  "Position: %s cm  Size: %s um" % (round(z,1), round(w,2) )
